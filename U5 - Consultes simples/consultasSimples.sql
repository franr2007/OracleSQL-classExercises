...
SELECT EDITORIAL, MAX(PREU) AS precioMasAlto FROM LLIBRES ORDER BY EDITORIAL WHERE PREU>30; --9

SELECT TITOL, AUTOR FROM LLIBRES WHERE LOWER(TITOL) LIKE 'm%' AND LOWER(AUTOR) LIKE '%h%'; --10

SELECT AUTOR, COUNT(*) AS cantidadLibros FROM LLIBRES GROUP BY AUTOR ORDER BY cantidadLibros DESC; --11

SELECT SUBSTRING(TITOL,1,1) AS primeraLetraTitulo, AVG(PREU) AS precioMedio FROM LLIBRES GROUP BY SUBSTRING(TITOL,1,1) ORDER BY primeraLetraTitulo; --12
